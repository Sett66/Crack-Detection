...
def get_args_parser():
    ...
    parser.add_argument('--BCELoss_ratio', default=0.67, type=float,
                        help='Weight ratio for Binary Cross Entropy Loss (0.0-1.0), should sum to 1 with DiceLoss_ratio')
    parser.add_argument('--DiceLoss_ratio', default=0.33, type=float,
    ...
    parser.add_argument('--dataset_path', default="/Crack_detection/LLH/Crack_datasets/DeepCrack_1",
...
def main(args):
    checkpoints_path = "./checkpoints/DALF/Albation/Modules"  # 模型
    cur_time = time.strftime('%Y_%m_%d_%H:%M:%S', time.localtime(time.time()))
    dataset_name = (args.dataset_path).split('/')[-1]
    checkpoints_path = os.path.join(checkpoints_path, dataset_name)  # 数据集
    process_folder_path = os.path.join(checkpoints_path, '无LFA')  # cur_time
    args.phase = 'train'
    ...
    output_dir = args.output_dir + '/' + dataset_name + '/' + 'DALF' + '/' + 'albation' + '/'+'Modules'+'/'+'无LFA'  # cur_time
    # output_dir = args.output_dir + '/' + "无cl"  # cur_time
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_root = f'./results/{dataset_name}/DALA/Albation/Modules/无LFA/{results_path}/results_' + str(epoch)
