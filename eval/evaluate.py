...
def get_image_pairs(data_dir, suffix_gt='lab', suffix_pred='pre'):
    ...
    for gt_path in gt_list:
        root = gt_path.replace(f"_{suffix_gt}.png", "")
        npy_path = root + f"_{suffix_pred}.npy"
        png_path = root + f"_{suffix_pred}.png"

        # 优先用概率图
        if os.path.exists(npy_path):
            pred = np.load(npy_path)   # float 概率图
        elif os.path.exists(png_path):
            pred = imread(png_path)    # 灰度/二值 PNG
        else:
            raise FileNotFoundError(f"预测文件不存在: {npy_path} or {png_path}")

        gt = imread(gt_path, thresh=127)  # 二值化标签

        pred_imgs.append(pred)
        gt_imgs.append(gt)
        pred_imgs_names.append(npy_path if os.path.exists(npy_path) else png_path)
...
def eval(log_eval, results_dir, epoch):
    ...
    return {'epoch': epoch, 'mIoU': mIoU, 'ODS': ODS, 'OIS': OIS, 'F1': F_list[0], 'Precision': Precision_list[0],
...
if __name__ == '__main__':
    suffix_gt = "lab"
    suffix_pred = "pre"
    results_dir = "/Crack_detection/LLH/MyProject/CLMamba/results/results_test/DSCNet/EdmCrack600_noval/"
    output_dir = "/Crack_detection/LLH/MyProject/CLMamba/eval/EdmCrack600_noval/"
    output_dir = os.path.join(output_dir, "DSCNet22.prf")
    logging.info(results_dir)
    src_img_list, tgt_img_list, pred_imgs_names, gt_imgs_names = get_image_pairs(results_dir, suffix_gt, suffix_pred)
    assert len(src_img_list) == len(tgt_img_list)
    final_accuracy_all = cal_prf_metrics(src_img_list, tgt_img_list)
    # final_semantaic_all = cal_semantic_metrics(src_img_list, tgt_img_list)

    # 新增将prf写入文件
    data_io.save_results(final_accuracy_all, output_dir)

    final_accuracy_all = np.array(final_accuracy_all)
    Precision_list, Recall_list, F_list = final_accuracy_all[:, 1], final_accuracy_all[:, 2], final_accuracy_all[:, 3]
    # global_acc, mean_acc, mean_iou_acc = final_semantaic_all[:, 1], final_semantaic_all[:, 2], final_semantaic_all[:, 3]
    mIoU = cal_mIoU_metrics(src_img_list, tgt_img_list, pred_imgs_names=pred_imgs_names, gt_imgs_names=gt_imgs_names)
    ODS = cal_ODS_metrics(src_img_list, tgt_img_list)
    OIS = cal_OIS_metrics(src_img_list, tgt_img_list)
